...
import { generateReferralPDF, downloadReferralPDF } from "@/utils/pdfGenerator";
...
const CreateReferralPage = () => {
  ...
  const fillSampleData = () => {
    ...
    setFormData({
      ...
      referring_facility: "Sunshine Birthing Center",
  ...
  const saveReferralDocument = async (referralId, referralData) => {
    try {
      ...
      const fileName = `referral_${selectedPatient?.first_name}_${selectedPatient?.last_name}_${new Date().toISOString().split('T')[0]}.pdf`;
      
      formDataToUpload.append('file', pdfBlob, fileName);
      formDataToUpload.append('document_type', 'referral');
      formDataToUpload.append('title', `Patient Referral - ${referralData.receiving_facility}`);
      formDataToUpload.append('description', `Referral form for ${selectedPatient?.first_name} ${selectedPatient?.last_name} to ${referralData.receiving_facility}`);
      formDataToUpload.append('patient_id', referralData.patient_id);
      formDataToUpload.append('referral_id', referralId);
      
      // Upload document
      await axios.post(`/api/birthcare/${birthcare_Id}/patients/${referralData.patient_id}/documents`, formDataToUpload, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
      
      console.log('Referral document saved successfully');
    } catch (error) {
          ...
          <div className="px-6 py-6 h-full flex flex-col">
            ...
            <div className="mb-6">
              <div className="text-center">
                <h1 className="text-2xl font-semibold text-gray-900 mb-2">Create Patient Referral</h1>
